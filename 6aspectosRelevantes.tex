...
En conclusión vimos que la ejecución en Android de esta aplicación iba a dar muchos problemas y determinamos que se programaría un servidor para que el cliente subiera ahí la foto y la biblioteca se ejecutará en el lado del servidor. Cabe destacar que después de un par de intentos de instalación del proyecto, los ejemplos, que estaban en la máquina Linux donde se alojaría el servidor, funcionaron de manera adecuada.
...
\begin{itemize}
    ...
    \item \textbf{SOAP:}\footnote{\url{http://www.cs.fsu.edu/~engelen/soap.html}} Esta especificación se tuvo que estudiar para comprender el funcionamiento de la herramienta GSOAP y en qué se basaba su funcionamiento, entender el porqué debía funcionar la herramienta y como se realiza la comunicación gracias a ella. Aunque el SOAP no es usado directamente cuando usas GSOAP es necesario conocer esta especificación ya que GSOAP sí que usa WSDL, para el cual tenemos que tener un conocimiento básico, al menos, de SOAP para poder usarlo.
    ...
    \begin{itemize}
    	\item La primera es usar un archivo WSDL donde especificas las operaciones que el  servidor va a realizar, después con la herramienta GSOAP generas todos los \textit{stubs} y documentos necesarios para hacer tu aplicación.
    	\item La segunda sería a través de un documento de tipo \cmd{.h} o una cabecera de C. Con el cuál también generas los stubs y documentos necesarios para programar tu servidor, entre dichos documentos se econtrará un archivo WSDL que contendrá la especificación de las operaciones que hay dentro del fichero cabecera que hayas usado. Pero incluso en esta opción necesitas entender SOAP y WSDL porque a través de comentarios tienes que especificar características que irán directamente al fichero WSDL, y que serán necesarios para el correcto funcionamiento del servidor.
    \end{itemize}
\end{itemize}

Una vez se ha estudiado lo anterior, se paso al estudio de la documentación de la herramienta GSOAP, además del intento de hacer que funcionen sus ejemplos. Cuando se consiguió que funcionarán sus ejemplos se paso a la programación de un servidor propio, una vez se programó y se hicieron las pruebas de que estaba bien programado, se procedió a intentar que este funcionará desde un cliente GSOAP.
Para que funcionará con el cliente GSOAP, se hizo una investigación de cómo hacer que el servidor funcionará en \textit{localhost} y, siguiendo la recomendación que en la documentación de GSOAP encontramos, se instaló Apache y se intentó usar el módulo de Apache para su funcionamiento con GSOAP.
...
\end{itemize}
Finalmente se consiguió integrar así el servidor con NeuralTalk, pero la carga de trabajo que esto llevo fue bastante grande.
...
El primer problema de todos es que para usar esta biblioteca era necesario instalarla de manera externa y en la documentación de Apache o Android no se encontraba la solución de manera clara. Se tuvo que investigar durante bastante tiempo hasta dar la solución en un foro de Internet\footnote{\url{http://stackoverflow.com/questions/28538078/java-lang-nosuchfielderror-org-apache-http-message-basicheadervalueformatter-in}}.
...
\subsection{Dificultades con biblioteca de traducción en el servidor}
\label{subchap:RelevTraduccion}
Tenemos dos problemas principales en la parte de traducción, que son los siguientes.
\subsection{Configuración en el servidor}
...
\subsection{Traducción obtenida}
Este problema se debe a que no disponemos de herramientas que trabajen directamente en español, por lo que tenemos que recurrir a la  traducción de Google.

El traductor de Google no es perfecto, por lo que estamos condicionados por su traducción, teniendo que tomar como premisa que dicha traducción es correcta. En algunos casos no es así, por ejemplo en la Figura \ref{traduc}, podemos ver que la traducción de la frase \textit{a man is holding a remote control in hand}, que creemos que es una predicción correcta, no tiene un sentido correcto con respecto a la imagen, mientras que en inglés sí que poseía ese sentido.
\figura{0.5}{imgs/probTrad.PNG}{Problemas al traducir la predicción}{traduc}{}